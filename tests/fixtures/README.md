...
Everything under `valid/` passes. Two documents under `invalid/` are rejected,
which is the point of them. The third, `ref-pointer-escape-tilde.json`, passes
...
| Fixture | Exercises | Validates | Pending |
| --- | --- | --- | --- |
...
| Fixture | Exercises | Validates | Pending |
| --- | --- | --- | --- |
| `notes-node-coverage.json` | one x-notes entry on each of: a tag, an operation, a parameter, a response, a schema and a schema property | yes | yes |
...
| `ref-nested-two-hops.json` | two-hop resolution: operation parameter -> components/parameters -> components/schemas, with keys on the inner target that must survive both hops | yes |  |
